 ...
 * Return: size of list
 */
list_dt *add_node(list_dt **head, const char *str, int nindex)
{
	list_dt *nhead;
	...
	if (!nhead)
		return (NULL);
	memset((void *)nhead, 0, sizeof(list_dt));
	nhead->nindex = nindex;
	if (str)
	{
 ...
 * Return: size of list
 */
list_dt *node_end(list_dt **head, const char *str, int nindex)
{
	list_dt *n_node, *node;
	...
	if (!n_node)
		return (NULL);
	memset((void *)n_node, 0, sizeof(list_dt));
	n_node->nindex = nindex;
	if (str)
	{
